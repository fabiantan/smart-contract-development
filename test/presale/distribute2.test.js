...
contract('payable', function (accounts) {
  ...
  before(async function () {
    ...
    // Time increase to 500 second after Presale start
    await time.increaseTo(preSaleStartTime + 500)
  ...
  it('Should return BNB', async function () {
    await time.increaseTo(preSaleEndTime + 30) // Set to 30 second just after presale finish
    await this.loaPresale.returnBNB(8, { from: owner, value: '0', gas: '5000000' });

    await time.increaseTo(distributeStartTime + 100 + 86400 * (3 - 1)) // Set to 3 days and 100 second just after distribute allowed
